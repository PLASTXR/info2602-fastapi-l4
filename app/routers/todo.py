...
@todo_router.post('/categories', response_model=CategoryResponse)
def create_category(db:SessionDep, user:AuthDep, category_data:CategoryCreate):
    ...
    try:
        ...
        db.commit()
        db.refresh(category)
        return CategoryResponse(id=category.id, text=category.text)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
def remove_category_from_todo(todo_id:int, cat_id:int, db:SessionDep, user:AuthDep):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="An error occurred while removing a category from the todo",
        )
